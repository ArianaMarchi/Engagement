...
if token and user_id:
    st.header("Panel de Analítica de Cursos", text_alignment="center")
    ...
    if mis_cursos:
        ...
        col1, col2, col3, col4 = st.columns([3,4,2,2], vertical_alignment="bottom")
        with col1:
        ...
        if id_discord and not id_discord.isdigit() or not (17 <= len(id_discord) <= 20):
            ...
            with col4:
                ...
                st.link_button("Conectar con Discord", url, type="primary", width="stretch")
        else:
            with col4:
                if st.button("Actualizar", 
                    type="primary", disabled=False, 
        ...
        with st.container(key="con_bordes", vertical_alignment="center"):
            row1_col1, row1_col2 = st.columns([3,2], width="stretch")
            row2_col1, row2_col2 = st.columns([4,3], width="stretch")
            with row1_col1:
